...
import (
	...
	"github.com/diginfra/diginfractl/pkg/test"
)
			...
			When("correct number of filepaths and platforms", func() {
				Context("pushing only one plugin: 1 filepath, 1 platform with multiple tags and with annotation source", func() {
					...
					It("should succeed", func() {
						...
						Expect(err).ToNot(HaveOccurred())
						Expect(manifest.Config.MediaType).To(Equal(oci.DiginfraPluginConfigMediaType))
						Expect(manifest.Layers).To(HaveLen(1))
						Expect(manifest.Layers[0].MediaType).To(Equal(oci.DiginfraPluginLayerMediaType))
		...
		Context("with filepaths", func() {
			When("only one rulesfile is given", func() {
				...
				It("should succeed", func() {
					...
					Expect(manifest.Layers[0].MediaType).To(Equal(oci.DiginfraRulesfileLayerMediaType))
					// It must have the rulesfile's config mediatype.
					Expect(manifest.Config.MediaType).To(Equal(oci.DiginfraRulesfileConfigMediaType))
				})
			})
	...
	Context("handling asset artifacts", func() {
		...
		When("only one asset tarball is given", func() {
			...
			It("should succeed", func() {
				...
				Expect(manifest.Layers[0].MediaType).To(Equal(oci.DiginfraAssetLayerMediaType))
				// It must have the asset config layer media type.
				Expect(manifest.Config.MediaType).To(Equal(oci.DiginfraAssetConfigMediaType))
			})
		})
