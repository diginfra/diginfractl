...
func (k *kmod) Cleanup(printer *output.Printer, driverName string) error {
	...
	if err == nil {
		...
		for i := 0; i < maxRmmodWait; i++ {
			...
			printer.Logger.Info("Nothing to do...'diginfractl' will wait until you remove the kernel module to have a clean termination.")
...
func (k *kmod) Load(printer *output.Printer, src, driverName string, fallback bool) error {
	if fallback {
		...
		if err == nil {
			...
			printer.Logger.Warn("Consider compiling your own driver and loading it or getting in touch with the Diginfra community.")
		}
		return err
	}
