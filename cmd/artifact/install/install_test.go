...
import (
	...
	out "github.com/diginfra/diginfractl/pkg/output"
)

//nolint:lll,unused // no need to check for line length.
var artifactInstallUsage = `Usage:
  diginfractl artifact install [ref1 [ref2 ...]] [flags]
...
Flags:
  ...
  -h, --help                              help for install
      ...
      --plugins-dir string                directory where to install plugins. (default "/usr/share/diginfra/plugins")
      --resolve-deps                      whether this command should resolve dependencies or not (default true)
      --rulesfiles-dir string             directory where to install rules. (default "/etc/diginfra")
...
Example - Install "latest" tag of "k8saudit-rules" artifact by relying on index metadata:
	diginfractl artifact install k8saudit-rules

Example - Install all updates from "k8saudit-rules" 0.5.x release series:
	diginfractl artifact install k8saudit-rules:0.5

Example - Install "cloudtrail" plugins using a fully qualified reference:
	diginfractl artifact install ghcr.io/diginfra/plugins/ruleset/k8saudit:latest
`
