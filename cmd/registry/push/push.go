...
const (
	longPush = `Push Diginfra "rulesfile" or "plugin" OCI artifacts to remote registry

Example - Push artifact "myplugin.tar.gz" of type "plugin" for the platform where diginfractl is running (default):
	diginfractl registry push --type plugin --version "1.2.3" localhost:5000/myplugin:latest myplugin.tar.gz

Example - Push artifact "myplugin.tar.gz" of type "plugin" for platform "linux/arm64":
	diginfractl registry push --type plugin --version "1.2.3" localhost:5000/myplugin:latest myplugin.tar.gz --platform linux/arm64

Example - Push artifact "myplugin.tar.gz" of type "plugin" for multiple platforms:
	diginfractl registry push --type plugin --version "1.2.3" localhost:5000/myplugin:latest \
...
Example - Push artifact "myrulesfile.tar.gz" of type "rulesfile":
	diginfractl registry push --type rulesfile --version "0.1.2" localhost:5000/myrulesfile:latest myrulesfile.tar.gz
...
Example - Push artifact "myrulesfile.tar.gz" of type "rulesfile" to an insecure registry:
	diginfractl registry push --type rulesfile --version "0.1.2" --plain-http localhost:5000/myrulesfile:latest myrulesfile.tar.gz
...
func NewPushCmd(ctx context.Context, opt *options.Common) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:                 "Push a Diginfra OCI artifact to remote registry",
		Long:                  longPush,
	...
	if len(artifactOptions.Requirements) != 0 {
		...
		for _, entry := range data {
			if requiredEngineVersionEntry, ok := entry[engineKey]; ok {
				...
				// int but internally used by diginfra as a semver minor version.
				// 15 -> 0.15.0
