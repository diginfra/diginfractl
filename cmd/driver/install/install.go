...
//nolint:gosec // this was an existent option in diginfra-driver-loader that we are porting.
...
func (o *driverInstallOptions) RunDriverInstall(ctx context.Context) (string, error) {
	o.Printer.Logger.Info("Running diginfractl driver install", o.Printer.Logger.Args(
	...
	if o.Distro.String() == driverdistro.UndeterminedDistro {
		if o.Compile {
			...
			o.Printer.Logger.Info(
				"Detected an unsupported target system, please get in touch with the Diginfra community. Trying to compile anyway.")
		} else {
			return "", fmt.Errorf("detected an unsupported target system, please get in touch with the Diginfra community")
		}
	}
