...
import (
	...
	drivercleanup "github.com/diginfra/diginfractl/cmd/driver/cleanup"
	driverconfig "github.com/diginfra/diginfractl/cmd/driver/config"
	driverinstall "github.com/diginfra/diginfractl/cmd/driver/install"
	driverprintenv "github.com/diginfra/diginfractl/cmd/driver/printenv"
	"github.com/diginfra/diginfractl/internal/config"
	driverdistro "github.com/diginfra/diginfractl/pkg/driver/distro"
	driverkernel "github.com/diginfra/diginfractl/pkg/driver/kernel"
...
func NewDriverCmd(ctx context.Context, opt *options.Common) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:                 "Interact with diginfra driver",
		Long:                  `Interact with diginfra driver.`,
...
func loadDriverVersion() string {
	...
	paths, _ := filepath.Glob("/usr/src/diginfra-*")
	for _, path := range paths {
		...
		drvVer := strings.TrimPrefix(filepath.Base(path), "diginfra-")
