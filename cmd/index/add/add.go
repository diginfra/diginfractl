...
func NewIndexAddCmd(ctx context.Context, opt *options.Common) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:                 "Add an index to the local diginfractl configuration",
		Long:                  "Add an index to the local diginfractl configuration. Indexes are used to perform search operations for artifacts",
