...
const (
	...
	diginfraHostRootEnvKey  = "HOST_ROOT"
)
...
func init() {
	...
	DiginfractlPath = filepath.Join(ConfigDir, "diginfractl")
	IndexesFile = filepath.Join(DiginfractlPath, "indexes.yaml")
	IndexesDir = filepath.Join(DiginfractlPath, "indexes")
	ClientCredentialsFile = filepath.Join(DiginfractlPath, "clientcredentials.json")
	DefaultIndex = Index{
		Name:    "diginfra",
	...
	DefaultDriver = Driver{
		...
		Name:     "diginfra",
		Repos:    []string{"https://download.diginfra.org/driver"},
		Version:  "",
...
func Load(path string) error {
	...
	viper.SetConfigName("diginfractl")
	...
	// Bind DIGINFRACTL_DRIVER_HOSTROOT key to HOST_ROOT,
	// so that we manage Diginfra HOST_ROOT variable too.
	_ = viper.BindEnv(DriverHostRootKey, diginfraHostRootEnvKey)
...
// "diginfra,https://diginfra.github.io/diginfractl/index.yaml;myindex,url"
...
// DiginfraVersions represent the map for Diginfra requirements
