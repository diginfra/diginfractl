...
PROJECT?=github.com/diginfra/diginfractl
...
fmt: gci addlicense
	go mod tidy
	go fmt ./...
	find . -type f -name '*.go' -a -exec $(GCI) write -s standard -s default -s "prefix(github.com/diginfra/diginfractl)" {} \;
	find . -type f -name '*.go' -exec $(ADDLICENSE) -l apache -s -c "The Diginfra Authors" -y "$(shell date +%Y)" {} \;
